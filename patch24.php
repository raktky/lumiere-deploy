<?php
/* patch24: apply q.php day-by-day render (patch23 never ran) + diagnostic + cleanup _d7. */
...
PHP;
    ...
    if($n>=1){
      ...
      else{ copy($f,$f.'.bak.patch24'); file_put_contents($f,$c2); $applied='applied'; }
    } else { $applied='replace0'; }
  }
}
/* diagnostic */
$d="patch24 render: $applied\n";
try{
  $r=db()->query("SELECT id, trip_request_id, token, title, LENGTH(itinerary) L FROM tb_quotes WHERE itinerary IS NOT NULL AND itinerary<>'' ORDER BY id DESC LIMIT 8");
  $d.="quotes WITH itinerary:\n";
  foreach($r->fetchAll(PDO::FETCH_ASSOC) as $x){ $d.="  ".json_encode($x)."\n"; }
}catch(Throwable $e){ $d.="dberr: ".$e->getMessage()."\n"; }
@unlink("$root/_d7_kx7.txt");
$d.="_d7 removed: ".(is_file("$root/_d7_kx7.txt")?'NO':'YES')."\n";
file_put_contents("$root/_d9_zt.txt",$d);
$cc=0;foreach(glob("$root/cache/*.html") as $g){@unlink($g);$cc++;} $d.="cache: $cc\n";
echo $d."DONE\n";
